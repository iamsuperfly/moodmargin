...
import { Menu, X, Zap, TrendingUp, LayoutDashboard, AlertTriangle } from "lucide-react";
...
export function Navbar() {
  ...
  // Auto-switch to Arbitrum Sepolia as soon as wallet connects
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Wrong-chain banner — shown below navbar when connected on non-Arbitrum-Sepolia */}
      {isWrongChain && (
        <div className="sticky top-16 z-40 w-full bg-amber-500/15 border-b border-amber-500/30 px-4 py-2">
          <div className="mx-auto max-w-7xl flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-amber-400 text-sm">
              <AlertTriangle className="w-4 h-4 shrink-0" />
              <span>Wrong network detected. MoodMargin requires <strong>Arbitrum Sepolia</strong>.</span>
            </div>
            <Button
              size="sm"
              className="shrink-0 bg-amber-500 hover:bg-amber-600 text-black text-xs font-semibold"
